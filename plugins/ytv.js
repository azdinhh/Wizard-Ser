...
let handler = async (m, { conn, args, isPrems, isOwner, usedPrefix, command }) => {
  if (!args || !args[0]) throw 'أوم ... أين عنوان url?'
  let chat = global.db.data.chats[m.chat]
  let server = (args[1] || servers[0]).toLowerCase()
  try {
    let { dl_link, thumb, title, filesize, filesizeF } = await ytv(args[0], servers.includes(server) ? server : servers[0])
    let isLimit = (isPrems || isOwner ? 99 : limit) * 1024 < filesize
    m.reply(isLimit ? `حجم الملف: ${filesizeF}\nلا يمكن تنزيل اكثر من  ${limit} MB, قم بتنزيله بنفسك: ${dl_link}` : global.wait)
    let _thumb = {}
    try { _thumb = { thumbnail: await (await fetch(thumb)).buffer() } }
    catch (e) { }
    if (!isLimit) conn.sendFile(m.chat, dl_link, '', `
*العنوان:* ${title}
*حجم الفيديو:* ${filesizeF}
  `.trim(), m, 0, {
      ..._thumb,
